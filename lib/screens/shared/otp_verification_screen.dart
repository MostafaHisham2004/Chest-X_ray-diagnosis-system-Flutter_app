...
class _OtpVerificationScreenState extends State<OtpVerificationScreen>
  ...
  Future<void> _sendOtp() async {
    ...
    if (auth.token == null) {
      _showSnack('You must be logged in to send a verification code.',
          isError: true);
      return;
    }
    ...
    try {
      ...
      _startCooldown();
      _showSnack('Verification code sent via WhatsApp!');
  ...
  Future<void> _verifyOtp() async {
    ...
    if (code.length != 6) {
      _showSnack('Enter the 6-digit code you received.', isError: true);
      _shakeCtrl.forward(from: 0);
      return;
    }
